...
```text
You are a senior enterprise architect. Create a BUSINESS architecture system diagram for a multi-tenant SACCOS SaaS platform.
...
- Platform type: Multi-tenant SACCOS management system (B2B SaaS).
- Main actors:
...
- Core business capabilities:
  - Tenant lifecycle and subscription plan assignment
  - Branch and staff provisioning
...
- Governance rules:
  ...
  - Tenant isolation across data and operations
...
- Business outcomes to show:
  ...
  - Scalable operations across tenants
...
- Backend:
  - Node.js + Express API
  - JWT auth middleware, RBAC middleware, subscription/feature/limit enforcement
  - Modules include auth, platform, tenants, users, members, loan applications, finance, dividends, auditor, reports, imports
...
- Show where rate limiting, RBAC, and subscription guards are enforced.
...
```text
Create a complete architecture pack for a multi-tenant SACCOS SaaS platform.
...
- Roles: platform owner, super admin, branch manager, loan officer, teller, auditor, member.
- Backend: Node/Express, Supabase admin integration, RBAC + subscription/feature/limit middleware.
...
- Core domains: tenant management, member onboarding, savings/shares, loans, dividends, cash control, reporting, auditing.
- Governance constraints: maker-checker, role-bound approvals, backend-enforced controls, tenant isolation.
