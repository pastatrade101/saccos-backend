...
router.post("/cycles", authorize([ROLES.BRANCH_MANAGER], { allowInternalOps: false }), requireFeature("dividends_enabled"), validate(createCycleSchema), controller.createCycle);
router.patch("/cycles/:id", authorize([ROLES.BRANCH_MANAGER], { allowInternalOps: false }), requireFeature("dividends_enabled"), validate(cycleParamSchema, "params"), validate(updateCycleSchema), controller.updateCycle);
